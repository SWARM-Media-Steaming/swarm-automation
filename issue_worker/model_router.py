...
The router's only job is choosing the least expensive, least token-intensive
provider/model/reasoning-effort combination reasonably expected to complete a
given task successfully — never simply the strongest model available. Model
selection and effort selection are scored together but are independently
...
class _BenchmarkNormalizer:
    ...
    catalog entries included or not.
    """
...
def _context_fit(model: ModelSpec, swe_atlas_norm: float | None) -> float:
    context_component = swe_atlas_norm if swe_atlas_norm is not None else model.relative_capability / 5.0
    return 0.4 * (model.relative_token_efficiency / 5.0) + 0.6 * context_component
...
def _score_candidate(
    ...
    *,
    ...
    """See the module docstring and skills/model-router/SKILL.md for the formula."""
    coding_capability, data_quality = _coding_capability(model, effort, group, normalizer)
    swe_atlas_norm = normalizer.normalize("swe_atlas_qna", (model.benchmarks.get(effort) or BenchmarkEntry(*([None] * 7), "HEURISTIC")).swe_atlas_qna)

    if model.relative_capability >= required_capability:
        expected_success = 1.0
    else:
        expected_success = max(0.15, model.relative_capability / required_capability) * 0.75
    ...
    components = {
        ...
        "cost_efficiency": (6 - model.relative_cost) / 5.0,
        "token_efficiency": model.relative_token_efficiency / 5.0,
    ...
    if request.cost_sensitive:
        weights["cost_efficiency"] = weights.get("cost_efficiency", 0.0) * rules.sensitivity_boost
    if request.token_sensitive:
        weights["token_efficiency"] = weights.get("token_efficiency", 0.0) * rules.sensitivity_boost
    ...
    unnecessary_reasoning = extra_effort_levels * rules.unnecessary_reasoning_penalty_per_level

    return positive - overqualification - unnecessary_reasoning
    ...
    *,
    ...
    for model in _eligible_models(catalog, availability):
        for effort in model.supported_efforts:
            ...
            scored.append(RoutingCandidate(model=model, effort=effort, score=score))
    ...
    tied = [c for c in scored if top_score - c.score <= rules.tie_break_margin]
    ...
    reason = _explain(winner, band, group, request, required_capability, min_effort)
...
def _explain(
    ...
    parts = [f"{band.level} {task_label} task."]
    if model.relative_capability <= required_capability:
        parts.append(f"{model.model} at {winner.effort} meets the required capability without paying for a larger model.")
    else:
        parts.append(f"{model.model} at {winner.effort} was the best-scoring option among what was available.")
    sensitivities = [
        label
        for flag, label in (
            (request.cost_sensitive, "cost"),
            (request.token_sensitive, "token usage"),
            (request.latency_sensitive, "latency"),
        )
        if flag
    ]
    if sensitivities:
        parts.append(f"Weighted for {', '.join(sensitivities)} sensitivity.")
    return " ".join(parts)
